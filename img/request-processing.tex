...
\begin{tikzpicture}[
  ...
  \node[draw,dashed,fill=colorrepeat,fill opacity=0.5, text opacity=1,below=4em of POD.north, minimum height=2cm, minimum width=21em] (ACP) {};
  \node[below right=0.5em and 0em, text depth=20mm,text height=3mm,title,text width=20em] at (ACP.north west){Access Control Policies {\footnotesize $QPK(q).p$}};

  \node[draw,fill=colorfile, text opacity=1,below left=3em and -9em of ACP, minimum height=5em, minimum width=9em] (f1) {};
  ...
  \draw[->,thick, dashed] (client) -- node[pos=0.4,above, xshift=-20, align=center] {request \\ $(i,q)$} (ACP.north);
