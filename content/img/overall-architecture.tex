...
\begin{tikzpicture}[
  ...
  %  \node[person,process,fill=colorperson] (ALICE) {Alice};
   \node[sink, right=of ALICE] (FE) {Federation \\ Engine}; 
  ...
  \node[title, below=1em of POD1] (KPOD1) {$K_1^S\dots K_n^S$};
  \node[title, below=1em of POD2] (KPOD2) {$K_1^R\dots K_n^R$};
  \node[title, below=1em of POD3] (KPOD2) {};
    ...
    \draw[to] (ALICE) -- node[midway,above] {$Q,K$} (FE);
    \draw[to] (IAM) -- node[midway,above] {Summaries} (FE);
