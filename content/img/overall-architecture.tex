...
\begin{tikzpicture}[
    ...
    \matrix{
      \node[process] (ALICE) {Alice}; \& \node[sink] (FE) {Federation \\ Engine}; \& \node[sink] (ACP) {AC Policy $\langle S,A,R\rangle$\\\\}; \& \node[db,cylinder,shape border rotate=90,shape aspect=.25] (DS) {Datastore};\\
      \node[process] (POD1) {Pods}; \& \node[sink] (POD2) {Pods}; \& \node[sink] (POD3) {Pods}; \&\\
    };
    \node[draw,fill=orange!30] at ([yshift=0.75cm]ACP.south){SHACL Shape};
    % Draw the arrows between the nodes and label them.
    \draw[to] (ALICE) -- node[midway,above] {Query, ID} (FE);
    \draw[to] (FE) -- node[midway,above] {Query, ID} (ACP);
